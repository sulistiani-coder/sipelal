<x-layouts.app title="Detail Peminjaman - {{ $loan->kode_pinjam }}">
    <div class="space-y-6">
        <a href="{{ route('riwayat.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-primary-600 dark:text-slate-400">
        ...
        <div class="rounded-2xl border border-primary-100 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">UUID: <code class="rounded bg-primary-50 px-1.5 py-0.5 text-xs text-primary-700 dark:bg-primary-900/30 dark:text-primary-400">{{ $loan->uuid }}</code></p>
                </div>
            ...
            <p class="mt-2 text-center text-xs text-slate-400">Tunjukkan QR ini saat pengambilan/pengembalian alat</p>
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    <p class="text-xs font-medium text-slate-500">Tanggal Kembali Aktual</p>
                    <p class="text-sm text-slate-900 dark:text-white">{{ $loan->tgl_kembali_aktual?->format('d M Y') }}</p>
                </div>
                @endif
                @if($loan->catatan)
                <div class="sm:col-span-2">
                    <p class="text-xs font-medium text-slate-500">Catatan</p>
                    <p class="text-sm text-slate-900 dark:text-white">{{ $loan->catatan }}</p>
                </div>
                @endif
            </div>
                    ...
                    @foreach($loan->loanItems as $item)
                        <div class="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3 dark:border-slate-600">
                            <div>
                                ...
                                <p class="text-xs text-slate-500">{{ $item->equipmentUnit->unit_code ?? '-' }} | Kondisi Pinjam: {{ $item->kondisi_saat_pinjam }}</p>
                            </div>
                            @if($item->kondisi_saat_kembali)
                                <div class="text-right">
                                    <p class="text-xs text-slate-500">Kondisi Kembali:</p>
